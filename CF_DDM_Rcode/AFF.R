...
AFF_long <- aggregate(AFF_long$respAffWord.corr, by=list(SubIndex = AFF_long$SubIndex,
  ...
  merge(AFF_long, by = c("SubIndex", 'congruency', 'AffValue'), all.x = TRUE)
...
Aff_wide_target$Conflict_pos <- Aff_wide_target$RT.target_Ic_neg - Aff_wide_target$RT.target_Ic_pos
Aff_wide_target$Conflict_neg <- Aff_wide_target$RT.target_Co_neg - Aff_wide_target$RT.target_Co_pos

AFF_wide <- merge(Aff_wide_prime, Aff_wide_target, by = 'SubIndex')

outers2 <- OuterDetectV(AFF_wide, SubIndex, c(2,15))
