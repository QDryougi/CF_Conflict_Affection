...
检验RT原则：依据subj_x / congruency / v alue / block分组筛选

### 1.1 离群值检测

```{r}
#| echo: false
CF_wide %>% OuterDetectN(SubIndex, switch_cost)
AFF_wide %>% OuterDetectN(SubIndex,  Ic_prime_grade, Conflict_pos)
```

```{r warning=FALSE}
#| echo: false
OUTERS <- c(11,23,16,33)
```
